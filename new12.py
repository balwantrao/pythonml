...
#intents = json.loads(open("dat.json",encoding="utf-8").read())

with open('dat.json', 'rb') as file:
    result = chardet.detect(file.read())
    encoding = result['encoding']

with open('dat.json','r',encoding=encoding) as file:
...
hist = model.fit(np.array(train_x), np.array(train_y),
				epochs=2000, batch_size=8, verbose=1)
